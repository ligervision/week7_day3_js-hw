...
function createAdder(x){ // use a closure which remembers its outer scope
    function add(y){
        return x + y
    }
    return add
...
// Exercise 2 - Promises 
// Using the below getMovieInfo function, which is a Promised-based function, write an asynchronous function (.then().catch() or async/await)
...
async function printMovieInfo(movieName){
    try{
        ...
        console.log(`${movie.title} - directed by ${movie.director}. ${movie.description}. Run time: ${movie.runtime} minutes.`)
    } catch(err) {
        console.warn(err)
    }
}
...
function printMovie(movieName){
    getMovieInfo(movieName)
        .then(movieInfo => console.log(`${movieInfo.title} - directed by ${movieInfo.director}. ${movieInfo.description}. Run time: ${movieInfo.runtime} minutes.`))
